...
public class Splash : MonoBehaviour
{
	...
	void OnHeadjackInitialized(bool succes, string error)
	{
		...
		if (!succes)
		{
			...
			{
				messageToShow = "<size=150%>This app is not published.</size>";
			}
			else
			{
				messageToShow= "<size=150%>You need an active internet connection</size>\n\nA working internet connection is required for initial set up";
			}
		...
		else
		{
			...
			{
				willShowError = true;
				messageToShow = "<size=150%>No available projects</size>\n\nIn order to use this application please create one or multiple projects on Headjack's App Page.\nProjects will be updated everytime you restart your app.";
			}
		}
